...
function updateSize(totalHeight) {
	...
	formSubmit.first('a').css('lineHeight',
			(((totalHeight / 7 * 2) / 5)) + 'px');
	formSubmit.children('div').css('marginTop', 10);

	var loginHeight = $(window).height() - header.height()
			- formSubmit.outerHeight(true);
	...
	// formLogin.children('div').find('input').height(((totalHeight / 6) /
	// 3)-5);
	// formLogin.children('div').find('input').css('line-height', (((totalHeight
	// / 6) / 3)-5) + 'px');
	// formLogin.children('div').find('input').css('min-height', (((totalHeight
	// / 6) / 3)-5) + 'px');
	...
	// formLogin.children('div').find('input').css('line-height', (size) +
	// 'px');
...
function doCheckInternetConnection() {
	window
			.checkInternetConnection(function(data) {
				if (data.internetConnectionAvailable) {
					...
					global
							.showPopup(
									...
									"Internet connection not available. Please enable online access",
									'error');
				}
			});
}
